...
app.get("/user/booknow", async (req, res) => {
    try {
        ...
        // });
        const cuisine = await new Promise((resolve, reject) => {
            connection.query("SELECT * FROM cuisine", async (error, result) => {
                if (error) {
                    reject(error);
                } else {
                    const modifiedResult = await Promise.all(result.map(async item => {
                        const cateringId = item.catetring_id;
                        const cateringInfo = await getCateringInfo(cateringId);
                        return {
                            ...item,
                            catering_name: cateringInfo ? cateringInfo.catering_name : null
                        };
                    }));
        ...
        // Function to get catering information based on catering_id
        const getCateringInfo = async (cateringId) => {
            return new Promise((resolve, reject) => {
                connection.query("SELECT * FROM catering WHERE catering_id = ?", [cateringId], (error, result) => {
                    if (error) {
                        reject(error);
                    } else {
                        resolve(result[0]); // Assuming catering_id is unique
                    }
                });
            });
        };
